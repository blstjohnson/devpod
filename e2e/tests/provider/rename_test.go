...
var _ = framework.DevPodDescribe("devpod provider rename", func() {
	...
	// RENAME-1
	ginkgo.It("should rename a provider to a new, valid name", ginkgo.Label("rename"), ginkgo.Ordered, func() {
	...
	// RENAME-2
	ginkgo.It("should fail to rename a provider to a name that already exists", ginkgo.Label("rename"), ginkgo.Ordered, func() {
		tempDir, err := framework.CopyToTempDir("testdata/simple-k8s-provider")
		framework.ExpectNoError(err)
		ginkgo.DeferCleanup(framework.CleanupTempDir, initialDir, tempDir)
	...
	// RENAME-3
	ginkgo.It("should fail to rename a non-existent provider", ginkgo.Label("rename"), ginkgo.Ordered, func() {
		f := framework.NewDefaultFramework(initialDir + "/bin")
	...
	// RENAME-4
	ginkgo.It("should rename a provider with an associated workspace", ginkgo.Label("rename"), ginkgo.Ordered, func() {
	...
	// RENAME-5
	ginkgo.It("should fail to rename a provider to an invalid name", ginkgo.Label("rename"), ginkgo.Ordered, func() {
		tempDir, err := framework.CopyToTempDir("testdata/simple-k8s-provider")
		framework.ExpectNoError(err)
		ginkgo.DeferCleanup(framework.CleanupTempDir, initialDir, tempDir)
